...
def NCEP_TPW(image):
    ...
    TPWcollection = ee.ImageCollection('NCEP_RE/surface_wv') \
        ...
        .map(lambda image: datedist(image, date))

    # Select the two closest model times
    closest = TPWcollection.sort('DateDist').toList(2)

    # Check if there is atmospheric data on the wanted day
    # If not, create a TPW image with non-realistic values
    ...
    # Add tpw to the image as a band
